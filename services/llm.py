...
async def parse_user_message(text: str, user_name: str, preferences: str | None, file_bytes: bytes = None,
    ...
    ТЫ ОБЯЗАН ВСЕГДА ВОЗВРАЩАТЬ ОТВЕТ СТРОГО В ФОРМАТЕ JSON, содержащем ЧЕТЫРЕ ключа:
    1. "reply": Твой текстовый ответ.
    2. "extracted_data": Массив объектов [{{"type": "workout"|"habit", "name": "название", "notes": "инфо"}}], если нужно сохранить данные. Иначе [].
    3. "new_preferences": Если из текста можно извлечь новые факты о пользователе, объедини их со старым досье и верни обновленный текст. Иначе null.
    4. "reminders": Массив объектов [{{"text": "текст", "remind_at": "YYYY-MM-DD HH:MM:SS"}}], если юзер попросил напомнить. Иначе [].
    """
